local UIShopGold=class("UIShopGold",function() return ccui.Layout:create() end)
local UIShopItem=require("src/ui_shopitem")
local g_path=BPRESOURCE("res/usercenter/")
require("bptools/class_tools")
function UIShopGold:ctor()
   print("hjjlog>>UIShopGold")
   self.list_item={}
   self.list_item_sleep={};
   self:init();

end
function UIShopGold:destory()
end


function UIShopGold:init()
...
function UIShopGold:clear_item()
...
function UIShopGold:get_a_gold_item()
...
function UIShopGold:update_layout()
    local l_product_table=json.decode(bp_get_product_data())
    --print("hjjlog>>uishopgold：",bp_get_product_data());
