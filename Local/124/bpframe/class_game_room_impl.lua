...
function class_game_room_impl:destory()
    ...
    if self._ptr_scene_game~=nil then 
        ...
        self._ptr_scene_game=nil;
        bind_function.set_game_frame(nil)
    end
end
...
function class_game_room_impl:init_game_room(param_room_data,param_code)
    if param_code==nil then 
        self._int_code=0
    else
        self._int_code=tonumber(param_code)
    end
    ...
    bp_socket_connect(self._the_game_socket, bp_get_route_address(), self._the_room_data.port);-- 127.0.0.1  demo.bookse.cn
end
function class_game_room_impl:init_game_room_with_code(param_room_data,param_code)
    --self._int_code=tonumber(param_code)
    
    self:init_game_room(param_room_data,param_code)
end
...
function class_game_room_impl:listener()
 ...
 function class_game_room_impl:on_socket_connect(index, code)
    ...
    if code ==0 then 
        ...
        print("hjjlog>>send_logon:",json.encode(struct_logon));   
...
function class_game_room_impl:on_socket_main_logon(mainCmdID, subCmdID, data)
    ...
    elseif subCmdID==V10.SUB_GAME_LOGON_ERROR then 
        ...
        bp_unpack_end();
        bp_show_hinting(bp_gbk2utf(l_error))
        --hjj_for_wait
...
function class_game_room_impl:on_socket_main_system(mainCmdID, subCmdID, data)
    if subCmdID==V10.SUB_SYSTEM_MESSAGE then 
       ...
       -- print("hjjlog>>SUB_SYSTEM_MESSAGE:",ptr_message.type);    
        if bit._and(ptr_message.type ,GF.KIND_MESSAGE_CLOSE) >0 then 
            --print("hjjlog>>SUB_SYSTEM_MESSAGE:111111111",bit._and(ptr_message.type ,GF.KIND_MESSAGE_CLOSE));
            self:send_data_to_client(GF.IPC_MAIN_CONCTROL,GF.IPC_SUB_CLOSE_FRAME,ptr_message)
        else 
          --  print("hjjlog>>SUB_SYSTEM_MESSAGE:222222222",bit._and(ptr_message.type ,GF.KIND_MESSAGE_CLOSE));
...
function class_game_room_impl:on_socket_main_frame_message(mainCmdID, subCmdID, data)
    local l_table={}
    l_table.wMainCmdID=mainCmdID;
    l_table.wSubCmdID=subCmdID;
    l_table.cbBuffer=data;
...
function class_game_room_impl:on_socket_sub_score(mainCmdID, subCmdID, data)
    ...
    if ptr_user_score.userid==bp_get_self_user_data(123).userid then 
        ...
        if bit._and(self._the_room_data.mode,GF.ROOM_MODE_REDPACKET)>0 then 
...
function class_game_room_impl:on_socket_sub_prop(mainCmdID, subCmdID, data)
        ...
        if table_data.prop~=nil then 
            for k, v in pairs(table_data.prop) do
                bp_set_self_prop_count(v.id,v.cnt);
            end
        end 
        bp_update_user_data(0);
end
...
function class_game_room_impl:start_game_client()
    ...
    bind_function.set_game_frame(self._ptr_scene_game)
    local l_this=self;
    self:on_frame_start();
end 
...
function class_game_room_impl:send_game_info_to_client()
    ...
    the_server_info.wKindID=self._the_room_data.gameid;
    the_server_info.wGameGenre=self._the_room_data.type;
    the_server_info.wChairCount=self._the_room_data.chaircount;
    the_server_info.wServerID=self._the_room_data.roomid;
